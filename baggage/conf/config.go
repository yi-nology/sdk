...
import (
	...
	"github.com/yi-nology/common/utils/xmapping"
	"log"
	"os"
	"path"
	"reflect"
...
var (
	fillDefaultUnmarshaler = xmapping.NewUnmarshaler(jsonTagKey, xmapping.WithDefault())
...
func LoadFromJsonBytes(content []byte, v any) error {
	...
	return xmapping.UnmarshalJsonMap(lowerCaseKeyMap, v, xmapping.WithCanonicalKeyFunc(toLowerCase))
}
...
func buildAnonymousFieldInfo(info *fieldInfo, lowerCaseName string, ft reflect.Type, fullName string) error {
	...
	case reflect.Map:
		elemField, err := buildFieldsInfo(xmapping.Deref(ft.Elem()), fullName)
		if err != nil {
			return err
		}
...
func buildFieldsInfo(tp reflect.Type, fullName string) (*fieldInfo, error) {
	tp = xmapping.Deref(tp)

	switch tp.Kind() {
	case reflect.Struct:
		return buildStructFieldsInfo(tp, fullName)
	case reflect.Array, reflect.Slice:
		return buildFieldsInfo(xmapping.Deref(tp.Elem()), fullName)
...
func buildNamedFieldInfo(info *fieldInfo, lowerCaseName string, ft reflect.Type, fullName string) error {
	...
	case reflect.Map:
		elemInfo, err := buildFieldsInfo(xmapping.Deref(ft.Elem()), fullName)
		if err != nil {
			return err
		}
